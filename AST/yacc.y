...
%type <nPtr> case_statement case_compound_statement print_statement

%type <nPtr> break_statement
...
print_statement
	: ALERT '(' STRING_LITERAL ')' ';' {$$ = opr(ALERT, 0);}
	;
...
case_compound_statement
	: case_statement {$$=$1;}
	| case_compound_statement case_statement {$$= opr(';', 2, $1, $2);}
	;
break_statement
	: BREAK ';' {$$ = opr(BREAK, 0);}
	;
case_statement
	: CASE '(' expression ')' ':' statement break_statement		{$$ = opr(CASE, 2, $3, $6);}
	| CASE '(' expression ')' ':' expression break_statement	{$$ = opr(CASE, 2, $3, $6);}
	| CASE '(' expression ')' ':' statement 	{$$ = opr(CASE, 2, $3, $6);}
	| CASE '(' expression ')' ':' expression 	{$$ = opr(CASE, 2, $3, $6);}
	| DEFAULT ':' statement 			{$$ = opr(DEFAULT, 1, $3);}
	| DEFAULT ':' expression 			{$$ = opr(DEFAULT, 1, $3);}
	;
...
iteration_statement
	...
	| SWITCH '(' expression ')' '{' case_compound_statement '}' 			{$$ = opr(SWITCH, 2, $3, $6); }
	;
...
external_declaration
	...
	| FUNCTION IDENTIFIER '(' ')' compound_statement {ex($5, 0); /*freeNode($2);*/}		
	;
